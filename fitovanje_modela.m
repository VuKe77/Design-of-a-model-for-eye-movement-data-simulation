...
raw_1 = raw_data(:,1);
raw_2 = raw_data(:,2);
sig1 = ukloni_impulsni_sum(raw_1,1000);
sig2 = ukloni_impulsni_sum(raw_2,1000);
raw_data = [sig1 sig2];
...
figure
    subplot(2,1,1)
        ...
        plot(t,raw_1)
        plot(t,sig1)
        title('Horizontalni stepen vizuelnog ugla[deg]') %svu - stepen vizuelnog ugla
        ylabel("Amplituda[deg]")
        xlabel('Vreme[s]')
        legend(["originalan siglan", "filtriran signal"])
     subplot(2,1,2)
...
DATA = detekcija_sakada1(raw_data,t);
%%
figure
    scatter(DATA.SACC.amplitudes,DATA.SACC.peak_vals)
    xlabel('Amplituda[deg]')
    ylabel("Maksimum brzine sakade[deg/s]")
...
figure
    ...
    xlabel('Maksimum brzine sakada[deg/s]');
    ylabel('Učestanost[n]');
